...
switch ($method) {
    case 'GET':
        ...
        if ($id > 0) {
            ...
            WHERE
                        contacts.`date` >=  '$date' and contacts.is_done = 0
            ORDER BY
            contacts.`date` ASC,
            contacts.`time` ASC";
    ...
    case 'POST':
        ...
        if ($action == 'delete' && $id > 0) {
            ...
            $is_in = isset($_POST['is_in']) ? $_POST['is_in'] : 0;
            $is_out = isset($_POST['is_out']) ? $_POST['is_out'] : 0;
            $in_card = isset($_POST['in_card']) ? intval($_POST['in_card']) : '';
            $out_card = isset($_POST['out_card']) ? intval($_POST['out_card']) : '';
            ...
            if ($id > 0) {
                ...
                $result = $con->query("Update contacts 
                        Set 
                        in_date = '$in_date',
                        in_time = '$in_time',
                        out_date = '$in_date',
